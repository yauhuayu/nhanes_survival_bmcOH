...
d.dtc[d.dtc ==99 | d.dtc ==9  | d.dtc==5 | d.dtc=="." ] <- NA
d.dtc[d.dtc ==1 | d.dtc ==3 | d.dtc==4 ] <- 0
d.dtc[d.dtc ==2] <- 1
tot.dtc <- apply(d.dtc,1,sum,na.rm=T)         

# ~~~~~~~~~~ need redundant ck ~~~~~~~~
tot.na <- apply(d.dtc,1,is.na) 
ck.dtc.na <- apply(tot.na,2,sum)  
tot.dtc[ck.dtc.na==28] <- NA

d.imp[d.imp ==99 | d.imp ==9 | d.imp==4 | d.imp==5 | d.imp=="." ] <- NA
d.imp[d.imp==1 | d.imp==2] <- 0
d.imp[d.imp==3] <- 1
tot.imp <- apply(d.imp,1,sum,na.rm=T)
length(tot.imp[tot.imp>0])     # 864 pt has 1777 implants
sum(tot.imp[tot.imp>0])

out.dent <- data.frame(cbind(d.dent$SEQN,d.dent$OHstatus,tot.dtc,ck.dtc.na,tot.imp))
colnames(out.dent) <- c("SEQN","OHstatus",colnames(out.dent)[3:5])
...
dentvisit4 <- rep(0,nrow(d.ohq))
dentvisit4[is.na(d.ohq$dentvisit)] <- NA
dentvisit4[d.ohq$dentvisit==9] <- NA
dentvisit4[d.ohq$dentvisit==77] <- NA
dentvisit4[d.ohq$dentvisit==99] <- NA
dentvisit4[d.ohq$dentvisit<=2 & !is.na(d.ohq$dentvisit)] <- 1        # last dentvisit <=1 year
dentvisit4[(d.ohq$dentvisit==3 | d.ohq$dentvisit==4) & !is.na(d.ohq$dentvisit)] <- 2       # last dentvisit >1 ; <=3 years
dentvisit4[(d.ohq$dentvisit==5 | d.ohq$dentvisit==6) & !is.na(d.ohq$dentvisit)] <- 3       # last dentvisit >3 years (including 5+)
dentvisit4[d.ohq$dentvisi==7 & !is.na(d.ohq$dentvisit)] <- 4       # last dentvisit = never
